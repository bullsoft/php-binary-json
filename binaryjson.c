...
/* If you declare any globals in php_binaryjson.h uncomment this:
...
const zend_function_entry binaryjson_functions[] = {
	PHP_FE(confirm_binaryjson_compiled,	NULL)		/* For testing, remove later. */
...
PHP_INI_BEGIN()
    STD_PHP_INI_ENTRY("binaryjson.global_value",      "42", PHP_INI_ALL, OnUpdateLong, global_value, zend_binaryjson_globals, binaryjson_globals)
    STD_PHP_INI_ENTRY("binaryjson.global_string", "foobar", PHP_INI_ALL, OnUpdateString, global_string, zend_binaryjson_globals, binaryjson_globals)
PHP_INI_END()
*/
/* }}} */
...
/* {{{ proto string confirm_binaryjson_compiled(string arg)
   Return a string to confirm that the module is compiled in */
PHP_FUNCTION(confirm_binaryjson_compiled)
{
	char *arg = NULL;
	int arg_len, len;
	char *strg;
	...
	len = spprintf(&strg, 0, "Congratulations! You have successfully modified ext/%.78s/config.m4. Module %.78s is now compiled into PHP.", "binaryjson", arg);
